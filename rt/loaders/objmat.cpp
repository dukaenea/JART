...
namespace {
    ...
    public:
        std::string name;
        float specularExp;
        MaterialInfo() : diffuse_(nullptr), ambient_(nullptr), specular_(nullptr), emissive_(nullptr), specularExp(1.0f) {}
        Texture* diffuse() { return diffuse_; }
        Texture* ambient() { return ambient_; }
        Texture* specular() { return specular_; }
        Texture* emissive() { return emissive_; }
        ...
        void emissive(Texture* newtex) { delete emissive_; emissive_ = newtex; }
    ...
    void matCreate(MatLib* dest, MaterialInfo& matinfo) {
        if (matinfo.name != "") {
            ...
            LambertianMaterial* emissiveLambertian = nullptr;
            Material* finalmat;
            if (matinfo.specular())
            ...
            if (matinfo.emissive()){
                //will only emit, 0 difusion
                finalmat = emissiveLambertian = new LambertianMaterial(matinfo.emissive(), new ConstantTexture(RGBColor::rep(0.f)));
            }
            if (phong && lambertian) {
                CombineMaterial* combine = new CombineMaterial();
                combine->add(phong, 1.0f);
                combine->add(lambertian, 1.0f);
                if(emissiveLambertian != nullptr)
                    combine->add(emissiveLambertian, 1.0f);
                finalmat = combine;
            }
...
void loadOBJMat(MatLib* dest, const std::string& path, const std::string& filename) {
    ...
    while(!matInput.eof())
    {
        ...
        else if(std::strncmp(cmd, "Kd", 2) == 0 || std::strncmp(cmd, "Ks", 2) == 0 || std::strncmp(cmd, "Ka", 2) == 0 || std::strncmp(cmd, "Ke", 2) == 0)
        {
            ...
            case 'e':
                material.emissive(new ConstantTexture(color)); break;
            }
        }
        ...
        else if(std::strncmp(cmd, "map_Kd", 6) == 0 || std::strncmp(cmd, "map_Ks", 6) == 0 || 
                std::strncmp(cmd, "map_Ka", 6) == 0 || std::strncmp(cmd, "map_Ke", 6) == 0) {
